...
from lib.envoy import fetch_envoy_data, parse_envoy_data, process_envoy_request
...
async def set_envoy_address(input_data: EnvoyAddressInput):
    try:
        ...
        return {
            "message": "Envoy Admin API address updated",
            "new_address": settings.ENVOY_ADMIN_API,
        }
...
async def parse_envoy(request: FileParseRequest, background_tasks: BackgroundTasks):
    background_tasks.add_task(
        process_parsing_task, request.files_or_dirs, request.filter_type
    )
    """
    ...
    try:
        ...
        for file in files:
            ...
            try:
                ...
                raise HTTPException(
                    status_code=400, detail=f"Invalid JSON in file {file.filename}"
                )
